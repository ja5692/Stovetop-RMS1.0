...
mintemp = 20                             #Minimum temperature required to trigger the "stove on" status
    ...
    if temperature <= mintemp:
        ...
        while True:
            ...
            if sms_obj is not None:
                ...
                if message == "ok":
                                           #hologram.sendMessage(json.dumps("Alert confirmed. Please take necesary action, your home is at risk"))
                    print "Alert confirmed. Please take necesary action, your home is at risk"
                    reply = hologram.sendSMS(phone, "Alert confirmed. Please take necesary action, your home is at risk")
                    break
                    ...
                    count = 0
                                           ...
                                           #hologram.sendMessage(json.dumps("No response recieved within 10 minutes, Alert Ignored. Reverting to standby mode."))
...
while True:
    ...
    if sms_obj is not None:                #If user sends something:
        ...
        if message.lower() in "status": #If user enters keyword
            update()
    time.sleep(1)
